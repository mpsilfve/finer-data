...
The complete data set consists of 170 manually annotated pages (248,544 word tokens) and 101 semi-manually annotated pages (211,034 word tokens). The counts of each named entity class in these sections are presented in Table \ref{tab: statistics}.
...
Obtained results on the OCR test set are presented in Table \ref{tab: precision recall and f-scores ocr}. The table contains two sets of values. The upper block presents the loss in recall caused by the OCR process and hence the upper limit to the NER performance. The lower block, meanwhile, shows the final NER performance taking into account both errors yielded by the OCR process and the Stanford NER system. 
