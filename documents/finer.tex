...
The rest of the paper is organized as follows. We first describe the corpus in Section \ref{sec: corpus}. We then present experimental results on the corpus using the Stanford Named Entity Recognizer \citep{} in Section \ref{sec: experiments}. Finally, conclusions on the work are presented in Section \ref{sec: conclusions}.
...
The data is presented in a two-column file format using the standard BIO notation. For example, consider the following sentence ''Applen Tim Cook myy iPhoneja suomalaisille.'' (''Tim Cook from Apple sells iPhones to the Finnish.''):
%\begin{table}[h!]
%\begin{small}
\begin{center}
\begin{tabular}{lc} 
...
Obtained results are presented in Table \ref{tab: precision recall and f-scores}. The system yielded an overall F1-score of 82.42. In what follows, we will discuss the individual class performances. \\
...
We described a new corpus consisting of Finnish news articles with a manually prepared named entity annotation. The corpus is freely available. In addition, we presented preliminary experimental results on the corpus using the Stanford Named Entity recognizer. The resulting system yielded a total F1-score of 82.42. In future work, we aim to develop NER systems with a higher performance.
