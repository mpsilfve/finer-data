...
Note that, according to English specifications \citep{verhagen2010}, the expressions can not begin with a Preposition or a clause of any type. Thus, in the following, only the bolded parts are considered valid English TIMEX3 expressions: before \textbf{Thursday}, in \textbf{the morning}, after the strike ended on \textbf{Thursday}, over \textbf{the last 2 years}. As for Finnish, we extend this rule to consider Postpositions as well as Prepositions. Thus, in the following, only the bolded parts are markable Finnish TIMEX3 expressions: ennen \textbf{torstaita} (before Thursday), \textbf{torstain} j\"alkeen (after Thursday). However, the Prepositions and Postpositions are allowed if they are incorporated in to the expression via inflection. Therefore, the following are allowed as Finnish TIMEX3 expressions: torstaina (on Thursday), kesŠllŠ (in the summer).
