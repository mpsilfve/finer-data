...
The set of named entity classes contains the three fundamental entity types, \textit{person}, \textit{organization}, \textit{location}, collectively referred to as the \textit{enamex} since MUC-6 competition \citep{grishman1996}. These three classes appear in the vast majority of published work on NER \citep{nadeau2007}. In addition, our annotation contains the classes \textit{products}, \textit{events}, \textit{temporal expressions}, and \textit{person titles}. Expand on this.
 
 Many of the entities marked here as products have been taken into account in previous work to some extent. For example,  in the English CoNLL-2003 data \citep{tjong2003}, programs are included in an entity class labeled as 'MISCELLANEOUS. Meanwhile, in   
 
Nester versus non-nested. \citep{} are nested, other are not.

Pure form versus derivations. To be markable, each class must appear as a whole and not as a derivation or as a part of a token. For example, \textit{Suomi} is considered as a markable location, whereas \textit{Suomi} and \textit{suomalainen} are not.  An important and quite commonly occurring exception to this rule is the set of cases, in which the compound word still refers to the same named entity: for example, consider ''Google-ohjelmistoyhti\"o'' (the software company Google) instead of simply ''Google'' or iPhone-puhelin (iPhone phone) instead of simply ''iPhone''. 

The entities can span multiple words, for example, "Sauli Niinist\"o" or ''Sony Pictures''. 

Coordination. "Windows- ja Linux-käyttöjärjestelmät". 
